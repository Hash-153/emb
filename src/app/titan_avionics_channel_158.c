/**
 * @file titan_avionics_channel_158.c
 * @brief Avionics Redundant Channel #158 Implementation
 */

#include "app/titan_avionics_channel_158.h"
...
titan_status_t titan_avionics_chan_158_init(titan_avionics_chan_158_t *chan)
{
    ...
    memset(chan, 0, sizeof(titan_avionics_chan_158_t));
    chan->channel_id = 158;
    chan->mode = TITAN_CHAN_158_MODE_IDLE;
    chan->healthy = TRUE;
    chan->heatsink_temp_c = 25.0f;
    titan_pid_init(&chan->channel_pid, 4.36f, 0.940f, 0.025f, 0.0f, -35.0f, 35.0f, 0.002f);
    return TITAN_OK;
}
...
titan_status_t titan_avionics_chan_158_step(titan_avionics_chan_158_t *chan, f32_t setpoint, f32_t feedback, f32_t dt)
{
    ...
    if (chan->mode == TITAN_CHAN_158_MODE_ACTIVE) {
    ...
    } else if (chan->mode == TITAN_CHAN_158_MODE_FAILSAFE) {
