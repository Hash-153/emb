/**
 * @file titan_avionics_channel_390.c
 * @brief Avionics Redundant Channel #390 Implementation
 */

#include "app/titan_avionics_channel_390.h"
...
titan_status_t titan_avionics_chan_390_init(titan_avionics_chan_390_t *chan)
{
    ...
    memset(chan, 0, sizeof(titan_avionics_chan_390_t));
    chan->channel_id = 390;
    chan->mode = TITAN_CHAN_390_MODE_IDLE;
    chan->healthy = TRUE;
    chan->heatsink_temp_c = 25.0f;
    titan_pid_init(&chan->channel_pid, 9.00f, 2.100f, 0.025f, 0.0f, -35.0f, 35.0f, 0.002f);
    return TITAN_OK;
}
...
titan_status_t titan_avionics_chan_390_step(titan_avionics_chan_390_t *chan, f32_t setpoint, f32_t feedback, f32_t dt)
{
    ...
    if (chan->mode == TITAN_CHAN_390_MODE_ACTIVE) {
    ...
    } else if (chan->mode == TITAN_CHAN_390_MODE_FAILSAFE) {
