/**
 * @file titan_subsystem_281.c
 * @brief Subsystem Controller #281 Logic
 */

#include "app/titan_subsystem_281.h"
#include <string.h>

titan_status_t titan_subsys_281_init(titan_subsys_281_t *sub)
{
    if (!sub) return TITAN_ERROR_NULL_POINTER;
    memset(sub, 0, sizeof(titan_subsys_281_t));
    sub->subsystem_id = 281;
