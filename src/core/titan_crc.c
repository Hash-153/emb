/**
 * @file titan_crc.c
 * @brief Optimized Table-Driven and Direct Bit-Serial CRC Checksum Implementations
 */

#include "titan_crc.h"

/* Precomputed CRC8 Table (Poly 0x07) */
static const u8_t s_crc8_table[256] = {
    
};

u8_t titan_crc8_calc(const u8_t *data, mem_size_t len, u8_t init_val)
{
    u8_t crc = init_val;
    if (!data) return 0;
    while (len--) {
        crc = s_crc8_table[crc ^ *data++];
    }
    return crc;
}

u8_t titan_crc8_dallas(const u8_t *data, mem_size_t len)
{
    u8_t crc = 0x00;
    if (!data) return 0;
    for (mem_size_t i = 0; i < len; ++i) {
        u8_t inbyte = data[i];
        for (u8_t j = 0; j < 8; ++j) {
            u8_t mix = (crc ^ inbyte) & 0x01;
            crc >>= 1;
            if (mix) {
                crc ^= 0x8C;
            }
            inbyte >>= 1;
        }
    }
    return crc;
}

u16_t titan_crc16_ccitt(const u8_t *data, mem_size_t len, u16_t init_val)
{
    u16_t crc = init_val;
    if (!data) return 0;
    while (len--) {
        crc = (u16_t)((crc << 8) ^ titan_crc8_calc((const u8_t[]){(u8_t)((crc >> 8) ^ *data++)}, 1, 0));
    }
    return crc;
}

u16_t titan_crc16_modbus(const u8_t *data, mem_size_t len)
{
    u16_t crc = 0xFFFF;
    if (!data) return 0;
    for (mem_size_t pos = 0; pos < len; pos++) {
        crc ^= (u16_t)data[pos];
        for (int i = 8; i != 0; i--) {
            if ((crc & 0x0001) != 0) {
                crc >>= 1;
                crc ^= 0xA001;
            } else {
                crc >>= 1;
            }
        }
    }
    return crc;
}

u16_t titan_crc16_xmodem(const u8_t *data, mem_size_t len)
{
    u16_t crc = 0x0000;
    if (!data) return 0;
    while (len--) {
        crc = (u16_t)(crc ^ (*data++ << 8));
        for (u8_t i = 0; i < 8; ++i) {
            if (crc & 0x8000) {
                crc = (u16_t)((crc << 1) ^ 0x1021);
            } else {
                crc = (u16_t)(crc << 1);
            }
        }
    }
    return crc;
}

static const u32_t s_crc32_table[16] = {
    0x00000000, 0x1DB71064, 0x3B6E20C8, 0x26D930AC,
    0x76DC4190, 0x6B6B51F4, 0x4DB26158, 0x5005713C,
    0xEDB88320, 0xF00F9344, 0xD6D6A3E8, 0xCB61B38C,
    0x9B64C2B0, 0x86D3D2D4, 0xA00AE278, 0xBDBDF21C
};

u32_t titan_crc32_step(u32_t current_crc, u8_t byte)
{
    u32_t crc = current_crc ^ byte;
    crc = (crc >> 4) ^ s_crc32_table[crc & 0x0F];
    crc = (crc >> 4) ^ s_crc32_table[crc & 0x0F];
    return crc;
}

u32_t titan_crc32_calc(const u8_t *data, mem_size_t len, u32_t init_val)
{
    u32_t crc = ~init_val;
    if (!data) return 0;
    for (mem_size_t i = 0; i < len; ++i) {
        crc = titan_crc32_step(crc, data[i]);
    }
    return ~crc;
}

u16_t titan_fletcher16(const u8_t *data, mem_size_t len)
{
    u16_t sum1 = 0;
    u16_t sum2 = 0;
    if (!data) return 0;
    for (mem_size_t i = 0; i < len; ++i) {
        sum1 = (u16_t)((sum1 + data[i]) % 255);
        sum2 = (u16_t)((sum2 + sum1) % 255);
    }
    return (u16_t)((sum2 << 8) | sum1);
}

u32_t titan_fletcher32(const u16_t *data, mem_size_t words)
{
    u32_t c0 = 0;
    u32_t c1 = 0;
    if (!data) return 0;
    for (mem_size_t i = 0; i < words; ++i) {
        c0 = (c0 + data[i]) % 65535;
        c1 = (c1 + c0) % 65535;
    }
    return (c1 << 16) | c0;
}
